...
[JetBrains Junie CLI](https://junie.jetbrains.com/) — inside an
isolated Docker container: one persistent container per workspace directory, per agent (selected with the `claude`,
`codex`, or `junie` subcommand). **This repo does not contain any agent's source.** Each agent is installed from npm (latest
version) into the image built from its own `Dockerfile` under `pi/`, `claude/`, `codex/`, or `junie/`. The only code here is the
`pipod` bash script and the four image definitions.

## Repository layout

| Path                                          | Role                                                                                                                                                       |
|-----------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `pipod`                                       | Bash entrypoint: builds the selected image, manages the per-workspace container, bootstraps config, runs `pi`, `claude`, `codex`, or `junie`. Parameterized by `$AGENT`. |
...
| `claude/Dockerfile`                           | Same base/user setup, plus `git` and `gh` (Claude Code's built-in GitHub workflows shell out to `gh`), `jq`, and `less`; no `fd` (Claude Code's Glob tool never shells out to it). Installs `@anthropic-ai/claude-code` globally. Used when invoked as `pipod claude`.                                      |
...
> `CODEX_HOME`); `auth.json` (auth) and `config.toml` are bootstrapped from the host's `~/.codex/`. For Junie the
> shared config lives under `$CONFIG_DIR/junie/` and is mounted at `/home/ubuntu/.junie` (Junie has **no config-dir
> env var**, so its fixed `~/.junie` home is mounted directly — the container `HOME` is `/home/ubuntu`);
> `config.json` (base settings), `settings.json` (user settings, which Junie resolves at HIGHER precedence than
> `config.json`), `allowlist.json` (action allowlist), and `secure_credentials.json` (Junie's credential store) are
> bootstrapped from the host's `~/.junie/`, as are the config DIRECTORIES `models/` (custom model profiles, which embed
> their own BYOK key/base URL and are referenced as `custom:<id>`), `mcp/`, `skills/`, `commands/`, and `agents/` — each
> copied recursively (file-by-file, only if missing) with the same localhost/127.0.0.1 → host.docker.internal rewrite.
> Junie's activation gate needs a JetBrains-Account credential (independent of the model profile); on the host it is
> usually in place with **no separate login** by reusing a JetBrains IDE/Toolbox sign-in. That credential lives in
> `~/.junie/secure_credentials.json` — which IS bootstrapped/mounted here, so it **carries across automatically** (no
> in-container login). The exception is hosts where Junie keeps the credential in the OS keyring instead (macOS Keychain /
> Windows Credential Manager / a Linux keyring Junie prefers): there is no file to copy, so log in once in the container
> (it then writes the shared `secure_credentials.json`), use `--auth`/`JUNIE_API_KEY`, or put provider keys in
> `config.json` `byok` (built-in provider models only). Per-workspace state
> is split by agent under `$CONFIG_DIR/workspaces/<ws>/<agent>/` (`pi`/`claude`/`codex`/`junie`): it holds that agent's
> per-project data (`sessions/`, plus `state/` and `log/` for Codex, `file-history/` for Claude, and `log/` for Junie, all bind-mounted
...
`$RUN_CMD`. Values below are for the default (`pi`) agent unless a `claude`-, `codex`-, or `junie`-specific note is given.

- **Container name**: `pipod-<sanitized_workspace_path>` (or `pipod-claude-<sanitized_workspace_path>` for Claude
  Code, `pipod-codex-<sanitized_workspace_path>` for Codex, or `pipod-junie-<sanitized_workspace_path>` for Junie), with a `-nonet` suffix under `--no-network`, derived
  from the invocation directory (`WORKSPACE_DIR="$(pwd)"`). Network mode and agent are both encoded in the name, so
  each workspace has up to eight independent containers (pi/claude/codex/junie × net/nonet) that share workspace & config but
  keep their own installed packages; switching `-nn`, `claude`, `codex`, or `junie` just uses the other container. `PROJECT_DIR`
...
- **Mounts** (set at `docker run`):
    ...
    - Shared config home (identical across every workspace): pi `$CONFIG_DIR` (`$HOME/.pipod`) → `/home/ubuntu/.pi`
      ...
      `AGENTS.md`, rules, `auth.json`); Junie `$CONFIG_DIR/junie` → `/home/ubuntu/.junie` (`config.json`,
      `settings.json`, `allowlist.json`, `secure_credentials.json`, plus the `models/`/`mcp/`/`skills/`/`commands/`/
      `agents/` config dirs; the JetBrains-Account credential for the activation gate lives in `secure_credentials.json`
      and carries across automatically — unless the host stores it in the OS keyring instead (then log in once in the
      container). See the Config bootstrap note).
    - Per-workspace **config override**, only if `workspaces/<ws>/<agent>/config` exists. For pi `…/pi/config` →
      ...
      `…/claude/config` → `/home/ubuntu/.claude`, Codex `…/codex/config` → `/home/ubuntu/.codex`, and Junie
      `…/junie/config` → `/home/ubuntu/.junie` the override shares
    ...
    - Per-workspace **data** (`workspaces/<ws>/<agent>/…`), bind-mounted over the matching path in the shared agent
      ...
      overlays all workspaces would share/clobber the same project state):
        ...
        - Junie `…/junie/sessions` → `/home/ubuntu/.junie/cli-sessions/sessions` (per-task transcripts:
          `events.jsonl`, `transcript.md`, `subagents/`); `…/junie/log` → `/home/ubuntu/.junie/logs` (app + upgrade
          logs).
...
- **Config bootstrap**: on run, missing essential files are copied from the host with `localhost`/`127.0.0.1`
  ...
  (auth)/`settings.json` from `~/.claude/`. Codex: `auth.json` (auth)/`config.toml` from `~/.codex/`. Junie:
  `config.json`/`settings.json`/`allowlist.json`/`secure_credentials.json` plus the `models/`/`mcp/`/`skills/`/`commands/`/`agents/`
  config directories from `~/.junie/`. Note Junie's activation gate needs a JetBrains-Account credential (independent
  of the model profile); it lives in `secure_credentials.json` and so **carries across automatically** when the host
  stores it there (no in-container login). The exception is hosts where Junie keeps it in the OS keyring instead (macOS
  Keychain / Windows Credential Manager / a Linux keyring Junie prefers) — then there's no file to copy, so log in once
  in the container (which writes the shared `secure_credentials.json`), use `--auth`/`JUNIE_API_KEY`, or `byok` in
  `config.json` for built-in provider models. The same bootstrap
...
shared config dir (`~/.pipod/agent` for pi, `~/.pipod/claude` for Claude, `~/.pipod/codex` for Codex, `~/.pipod/junie` for Junie) and that agent's
per-workspace data dirs/files under `~/.pipod/workspaces/<ws>/<agent>/` (`sessions/` for all; `state/` + `log/` for
Codex; `file-history/` for Claude; `log/` for Junie; plus the `history.jsonl` file for Claude/Codex), and bootstraps the agent's essential
config (`models.json`/`auth.json` for pi; `.credentials.json`/`settings.json` for Claude; `auth.json`/`config.toml` for
Codex; `config.json`/`settings.json`/`allowlist.json`/`secure_credentials.json` for Junie) from the host if missing.
...
- **This repo wraps agents, it doesn't fork them**: behavior changes to pi, Claude Code, Codex, or Junie itself belong
  upstream, or in their own config/extensions — not as patches here. Each agent's own environment variables and
  config-file locations (e.g. `CLAUDE_CONFIG_DIR`, `.credentials.json`, `CODEX_HOME`, `auth.json`, Junie's fixed
  `~/.junie` home) are relied upon,
  not reimplemented.
